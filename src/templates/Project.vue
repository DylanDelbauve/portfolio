<template>
  <layout>
    <div class="w-screen min-h-screen flex justify-center">
      ...
      <g-image :src="$page.project.thumbnail[0].url" width="800" class="invisible md:visible w-full h-full z-0 fixed object-cover object-center blur-md contrast-75"/>
      <div class="h-full min-h-screen w-full md:w-10/12 lg:w-3/5 flex flex-col justify-center items-center z-10 bg-gray-700 py-28 md:py-24 px-12 shadow-2xl">
        <g-image :src="$page.project.thumbnail[0].url" width="800" class="w-full rounded-2xl shadow object-fill"/>
        <div class="w-full mx-auto flex my-4 py-4 gap-4 md:flex-row flex-col">
          ...
          <div class="w-full flex gap-2">
            <p v-for="tag in $page.project.tags" :key="tag" class="py-2 px-4 bg-sky-900 rounded-3xl text-white font-bold shadow w-git h-fit">
              {{tag}}
            </p>
          </div>
        </div>
...
<page-query>
query($id: ID!) { project: project(id : $id) {title, description, thumbnail {url}, tags} }
</page-query>
